...
function Counter({ value, suffix = "", label }: CounterProps) {
  ...
  return (
    <div ref={ref} className="text-center">
      <div className="text-4xl md:text-6xl font-bold text-navy mb-2 flex items-center justify-center">
        <motion.span>{rounded}</motion.span>
        <span>{suffix}</span>
      </div>
      <p className="text-charcoal/60 font-medium">{label}</p>
      <motion.div
        initial={{ width: 0 }}
        whileInView={{ width: "40px" }}
        viewport={{ once: true }}
        transition={{ duration: 0.8, delay: 2 }}
        className="h-1 bg-coral mx-auto mt-4"
      />
    </div>
  );
}
